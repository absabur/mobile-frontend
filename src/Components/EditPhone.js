...
import { buildSpecifications, reverseSpecifications } from "@/util";
import { useEffect, useState } from "react";
import { useDispatch, useSelector } from "react-redux";

export default function EditPhone({ phoneDetails }) {
  ...
  useEffect(() => {
    ...
    setPhone((prev) => ({ ...prev, name: phoneDetails.name }));
    setPhone((prev) => ({ ...prev, price: phoneDetails.price }));
    setVarientCount(phoneDetails.price.length);
    setPhone((prev) => ({
      ...prev,
      brand: {
        _id: phoneDetails.brand.id,
        name: phoneDetails.brand.value,
      },
    }));
    setImagesPreview(phoneDetails?.images);
    setSpecs(reverseSpecifications(phoneDetails.specifications).array1);
    setValue(reverseSpecifications(phoneDetails.specifications).array2);
  }, []);
  ...
  const post = () => {
    ...
    dispatch(editPhoneAction(phoneDetails._id, myForm));
  };
  ...
  return (
    <div>
      <h1 style={{ textAlign: "center", marginTop: "10px" }}>Update phone</h1>
      <div className="add-phone-form">
        <input
          value={phone?.name}
          type="text"
          placeholder="Enter phone name"
          onChange={(e, phone) => setPhone({ ...phone, name: e.target.value })}
        />
